...
class CyberTAgent:
    ...
    async def run(
        ...
        try:
            ...
            log.warning("CyberT: erro de status da API", status=e.status_code, overloaded=is_overloaded)
            yield StreamEvent(agent=AgentName.CYBERT, type=EventType.ERROR, content=msg)
            yield StreamEvent(agent=AgentName.CYBERT, type=EventType.COMPLETE, content="")
